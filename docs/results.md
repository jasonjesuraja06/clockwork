...
Read this before the tables. Three facts qualify every measurement on this page.

1. The single-turn rows are synthetic, not ShareGPT. The five workloads now named
   `synthetic_singleturn_r1` through `synthetic_singleturn_r16` were generated by the seeded
   lognormal length sampler in `clockwork/bench/workloads.py` over a 210-word vocabulary. They
...
2. The agent traces are self-designed by this project. They are not a public dataset and not a
   ...
   prefix cache exists to exploit. A benchmark shaped around the mechanism under test favors that
   mechanism by construction, so read the agent-workload win as evidence that the prefix cache
   works on the traffic it targets, not as a measurement of anyone else's workload.
3. Both engines were driven by this repository's own harness. `scripts/run_bench.py` generated
   the traces, issued the requests, and computed every metric, for clockwork and for vLLM alike.
   No independent benchmark tool has corroborated any number here, and a harness bug would move
   both arms together without showing up in the comparison.
...
follow the workload's Poisson or Pareto process at the configured rate. The single-turn
generator warns on stderr whenever it falls back to synthesizing lengths, which is what every
run behind this page did. Metrics, as computed by `clockwork/bench/metrics.py`:
...
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
| synthetic_singleturn_r1 | singleturn | 1 | on | 113.2 | 539.8 | 31.3 | 95.9 | 111.7 | 0.052 |
| synthetic_singleturn_r2 | singleturn | 2 | on | 121.6 | 485.5 | 32.8 | 136.4 | 195.9 | 0.048 |
| synthetic_singleturn_r4 | singleturn | 4 | on | 170.9 | 966.0 | 45.0 | 248.1 | 317.5 | 0.045 |
| synthetic_singleturn_r8 | singleturn | 8 | on | 260.4 | 3641.2 | 65.7 | 310.7 | 466.2 | 0.052 |
| synthetic_singleturn_r16 | singleturn | 16 | on | 1647.9 | 11883.4 | 65.8 | 416.4 | 499.0 | 0.052 |
...
Every trace in this table is synthetic and generated by this repository. The `singleturn`
rows came from the seeded length sampler, not from the ShareGPT dataset; the `agent` and
`ablation` rows are self-designed to model agent-loop traffic, and their long shared
prefixes are the structure this engine is built to exploit. See the disclosure above.
...
"Same harness" is the load-bearing limitation of this comparison. `scripts/run_bench.py`
generated the traces, drove both servers, and computed both engines' metrics, so the two arms
share every source of harness error, and no independent benchmark tool has checked the result.
The traces themselves are this project's own: synthetic single-turn lengths, and self-designed
agent traffic whose long shared prefixes are what clockwork is built to exploit.

| workload | kind | req/s | radix | ttft p50 (ms) | ttft p99 (ms) | itl p50 (ms) | itl p99 (ms) | out tok/s | hit rate |
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
| synthetic_singleturn_r1 | singleturn | 1 | on | 96.7 | 1153.9 | 16.7 | 50.4 | 92.4 | 0.052 |
| synthetic_singleturn_r2 | singleturn | 2 | on | 100.7 | 591.7 | 17.3 | 81.0 | 156.2 | 0.048 |
| synthetic_singleturn_r4 | singleturn | 4 | on | 138.5 | 1428.3 | 20.1 | 163.6 | 288.1 | 0.045 |
| synthetic_singleturn_r8 | singleturn | 8 | on | 162.6 | 541.1 | 28.7 | 266.0 | 502.4 | 0.052 |
| synthetic_singleturn_r16 | singleturn | 16 | on | 834.5 | 3024.1 | 53.7 | 508.6 | 646.4 | 0.052 |
...
Mean output tok/s over the 12 self-designed synthetic agent traces: clockwork 245.2 versus
vLLM 168.7, with clockwork faster on 12 of 12 by 1.17x to 2.38x. That range is a win on
traces this project designed around its own prefix cache, scored by this project's harness.
It is not a general claim about vLLM, and it should not be read as one. On the five synthetic
single-turn rates the order reverses: clockwork 318.0 versus vLLM 337.1, vLLM
